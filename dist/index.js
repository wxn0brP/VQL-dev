...
export function setup_VQL_DEV(app, processor) {
...
export function isValtheraInstance(db) {
    return (typeof db?.dbAction === "object" &&
        typeof db?.executor === "object" &&
        !("meta" in db));
}
export function getAdapterMeta(id, db) {
    const adapter = {
        logic_id: id,
        type: "unknown",
        version: "0.0.0",
...
export function getAdapterHTTP(processor) {
    return (req, res) => {
        try {
...
export function getAdaptersHTTP(processor) {
...
export class VqlDevPanel {
    ...
    _setupHTTP() {
        ...
        this._app.get("/", () => "Still running. Must've missed the shutdown memo.");
    }
    start() {
        if (this._started)
