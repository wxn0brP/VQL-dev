...
function hasRequiredParams(): boolean {
	...
	return (
		p.has("server") || p.has("url") || p.has("s") || p.has("p") || p.has("port")
	);
}
...
class ServerConfigView implements UiComponent {
	...
	mount() {
		...
		this.form
			...
			.forEach(radio => {
				radio.addEventListener("change", () =>
					this._onModeChange(radio.value as Mode),
				);
			});
		...
		this.form
			.querySelectorAll<HTMLButtonElement>(".kv-add-btn")
			.forEach(btn => {
		...
		if (!hasRequiredParams()) this.open();
	}
	...
	_apply() {
		...
		if (mode === "web") {
			...
			if (endpoint && endpoint !== "/VQL") params.set("ep", endpoint);
...
window.addEventListener("hashchange", () => {
	if (location.hash !== lastHash) location.reload();
});
