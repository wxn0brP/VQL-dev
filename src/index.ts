...
export function setup_VQL_DEV(app: Router, processor: VQLProcessor) {
	...
	router.get(
		"/health",
		() => "Life expectancy, Hide and Seek, Castling, Scars",
	);
...
export function getAdapterMeta(
	id: string,
	db: ValtheraCompatible,
): ValtheraResolverMeta {
...
export function getAdaptersHTTP(processor: VQLProcessor): RouteHandler {
	return async (req, res) => {
		try {
			return Object.keys(processor.dbInstances).map(key =>
				getAdapterMeta(key, processor.dbInstances[key]),
			);
		} catch (err: any) {
			res.status(500).json({
				err: true,
...
export class VqlDevPanel {
	...
	public start() {
		...
		this._app.listen(this._port, () => {
			console.log(
				`[DevPanelBackend] Running at http://localhost:${this._port}`,
			);
		});
